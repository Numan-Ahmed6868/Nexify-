  ...
  return (
    <div ref={containerRef} className="relative z-10 pt-32 pb-24 px-6">
      <div className="max-w-7xl mx-auto space-y-32">
          ...
          >
            <div className="aspect-video glass rounded-[40px] overflow-hidden relative group">
              ...
              <div className="absolute bottom-8 left-8">
                ...
                <h3 className="text-2xl font-bold">Punjab, Lahore</h3>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        {/* Stats */}
        <motion.div style={{ y: statsY }} className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {stats.map((stat, i) => (
            <motion.div
              key={i}
              ...
              className="glass p-8 rounded-3xl text-center space-y-2 tilt-card neon-glow-blue"
            >
              <p className="text-4xl md:text-5xl font-black text-brand-primary">{stat.value}</p>
              <p className="text-xs font-bold tracking-widest opacity-40 uppercase">{stat.label}</p>
            </motion.div>
          ))}
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="glass p-12 rounded-[40px] space-y-6 tilt-card"
          >
        ...
        <div className="space-y-16">
          <div className="text-center space-y-4">
            ...
            <p className="opacity-50 max-w-2xl mx-auto">The principles that guide everything we do at Nexify.</p>
          </div>
          ...
          <div className="max-w-2xl space-y-8 relative z-10">
            ...
            <div className="flex items-center gap-4">
              <div className="flex -space-x-4">
                {[1, 2, 3, 4, 5].map((n) => (
                  <div key={n} className="w-12 h-12 rounded-full border-4 border-bg-dark bg-brand-primary/10 flex items-center justify-center text-xs font-bold">
                    TEAM
                  </div>
                ))}
              </div>
              <p className="text-sm font-bold opacity-40 tracking-widest uppercase">Join our growing family</p>
            </div>
          </div>
        </div>
      </div>
